...
void makefill(k, C, constraints, pfill, work1, printlevel) int k;
...
int printlevel;
{
      ...
      case MATRIX:
        for (i = 1; i <= C.blocks[blk].blocksize; i++)
          for (j = 1; j <= C.blocks[blk].blocksize; j++) {
            if ((C.blocks[blk].data.mat[ijtok(i, j, C.blocks[blk].blocksize)] !=
                 0.0) ||
                (i == j))
              work1.blocks[blk].data.mat[ijtok(i, j, C.blocks[blk].blocksize)] =
                  1.0;
          };
        break;
      case PACKEDMATRIX:
      ...
      switch (C.blocks[blk].blockcategory) {
        ...
        case MATRIX:
          for (j = 1; j <= ptr->numentries; j++) {
            p = ptr->iindices[j];
            q = ptr->jindices[j];
            work1.blocks[blk].data.mat[ijtok(p, q, C.blocks[blk].blocksize)] =
                1.0;
            work1.blocks[blk].data.mat[ijtok(q, p, C.blocks[blk].blocksize)] =
                1.0;
          };
          break;
        case PACKEDMATRIX:
  ...
  ptr = (struct sparseblock *)malloc_d(sizeof(struct sparseblock));
  ...
  for (i = 2; i <= C.nblocks; i++) {
    ptr->next = (struct sparseblock *)malloc_d(sizeof(struct sparseblock));
  ...
  while (ptr != NULL) {
    ...
    switch (C.blocks[blk].blockcategory) {
      case DIAG:
        ...
        ptr->entries =
            (double *)malloc_d((ptr->blocksize + 1) * sizeof(double));

        ptr->iindices = (int *)malloc_d((ptr->blocksize + 1) * sizeof(int));
        ...
        ptr->jindices = (int *)malloc_d((ptr->blocksize + 1) * sizeof(int));
      ...
      case MATRIX:
        ...
        ptr->entries =
            (double *)malloc_d((ptr->numentries + 1) * sizeof(double));

        ptr->iindices = (int *)malloc_d((ptr->numentries + 1) * sizeof(int));

        ptr->jindices = (int *)malloc_d((ptr->numentries + 1) * sizeof(int));
  ...
  while (ptr != NULL) {
    ...
    if (printlevel >= 3)
      printf("Block %d, Size %d, Fill %d, %.2f \n", blk,
             C.blocks[blk].blocksize, ptr->numentries,
             100.0 * ptr->numentries /
                 (C.blocks[blk].blocksize * C.blocks[blk].blocksize * 1.0));
    ptr = ptr->next;
  };
}
