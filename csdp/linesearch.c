...
double linesearch(n, dX, work1, work2, work3, cholinv, q, z, workvec, stepfrac,
                  start, printlevel) int n;
...
int printlevel;
{
  ...
  lanczosvectors = (double *)malloc_d((LANCZOSITS + 1) * n * sizeof(double));
  ...
  for (i = 1; i <= n; i++) q[i] = 1.0 / sqrt(n * 1.0);

  for (i = 1; i <= n; i++) lanczosvectors[ijtok(i, 1, n)] = q[i];
  ...
  for (j = 1; j <= LANCZOSITS; j++) {
    ...
    lalpha[j] = 0.0;
    for (i = 1; i <= n; i++) lalpha[j] = lalpha[j] + q[i] * z[i];
    ...
    MY_DGEMV("T", &n, &j, &scale1, lanczosvectors, &n, z + 1, &inc, &scale2,
             reorth + 1, &inc);

    scale1 = -1.0;
    scale2 = 1.0;
    inc = 1;
    MY_DGEMV("N", &n, &j, &scale1, lanczosvectors, &n, reorth + 1, &inc,
             &scale2, z + 1, &inc);
    ...
    if (fabs(lbeta[j]) < 1.0e-16) {
      if (printlevel >= 3) printf("Small beta[j]\n");
      j = j - 1;
      jj = j;
      goto DONEEARLY;
    };

    for (i = 1; i <= n; i++) q[i] = z[i] / lbeta[j];

    /*
     * Store the Lanczos vector.
     */

    for (i = 1; i <= n; i++) lanczosvectors[ijtok(i, j + 1, n)] = q[i];
    ...
    if (j >= 5) {
      ...
      for (i = 1; i <= j - 1; i++) lbeta2[i] = lbeta[i] * lbeta[i];
      for (i = 1; i <= j; i++) evalues[i] = lalpha[i];
      ...
      for (i = 1; i <= j; i++) {
        if (printlevel >= 4) printf("qreig evalue %e \n", evalues[i]);
        if (evalues[i] > maxeigs[j]) maxeigs[j] = evalues[i];
      };

      if (maxeigs[j] > maxeig) maxeig = maxeigs[j];
    };
    ...
    if ((j >= 7) && (maxeigs[j] <= 1 / (3 * start)) &&
        (fabs((maxeigs[j] - maxeigs[j - 2]) / (0.000001 + fabs(maxeigs[j]))) <
         0.2)) {
      if (printlevel >= 4) printf("Stopping on <1/3s j=%d \n", j);
      jj = j;
      goto DONEEARLY;
    };

    if ((j >= 8) && (fabs((maxeigs[j] - maxeigs[j - 2]) /
                          (0.000001 + fabs(maxeigs[j]))) < 0.02)) {
      if (printlevel >= 4) printf("Stopping here, on tightness j=%d \n", j);
...
DONEEARLY:
  ...
  if (printlevel >= 4) {
    for (i = 1; i <= jj; i++) printf("maxeigs[%d]=%e \n", i, maxeigs[i]);
    printf("maxeig %e \n", maxeig);
  };

  if (printlevel >= 4) printf("Lancoz converged after %d iters\n", jj);
