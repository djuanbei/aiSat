...
void copy_mat(A, B) struct blockmatrix A, B;
{
  int blk;
  double *p;
  double *q;
  ...
  for (blk = 1; blk <= A.nblocks; blk++) {
    switch (A.blocks[blk].blockcategory) {
      case DIAG:
        ...
        memcpy(p + 1, q + 1, A.blocks[blk].blocksize * sizeof(double));
      ...
      case MATRIX:
        ...
        memcpy(
            q, p,
            A.blocks[blk].blocksize * A.blocks[blk].blocksize * sizeof(double));
