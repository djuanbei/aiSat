...

{
    ...
    pC->nblocks=nblocks;
    pC->blocks=(struct blockrec *)malloc_d((nblocks+1)*sizeof(struct blockrec));
    ...
    myconstraints=(struct constraintmatrix *)malloc_d((*pk+1)*sizeof(struct constraintmatrix));
        ...
        for (blk=1; blk<=nblocks; blk++)
        {
            ...
            *pn=*pn+abs(blksz);
            ...
            if (blksz < 0)
            {
                ...
                pC->blocks[blk].data.vec=(double *)malloc_d((1+abs(blksz))*sizeof(double));
            ...
            else
            {
                ...
                pC->blocks[blk].data.mat=(double *)malloc_d((blksz*blksz)*sizeof(double));
