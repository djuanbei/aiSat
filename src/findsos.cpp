...
static void delpoly(void *p) { deletePoly((Poly *)p); }

static void sosPresent(const SubPoly *subpoly, const PointList *sosList,
...
static int overConvexChecking(const SubPoly *subpoly, indice_t *indices,
                              const int loc[], const int size,
                              const map<monomial, monomialvalue> &monMap,
                              PointList *ans);

static int exactConvHull(const SubPoly *subpoly, indice_t *candidateSet,
...
static Constraintmatrix *createConstraintMatrix(const SubPoly *subpoly,
  ...
  *b = (double *)calloc_d((sosLength + 1), sizeof(double));
  ...
  for (i = 0; i < size; i += 1) {
    //		cout	<< "vertex num %" <<VERTEX_NUM<<"
    //"<<monMap.find(indices+loc[i]*varNum).key().size()<< endl;
...
static int overConvexChecking(const SubPoly *subpoly, indice_t *indices,
  ...
  while (i < checkSize) {
    if (!checked[check[i]]) {
      ...
      if (num < size) {
        if (NULL != ans) cout << "found a split poly" << endl;
