...
constraint_left  SOSEQ constraint_right
{
  $$=$1;
...
{  $$=$2; }
|
'-' constraint_left %prec uminus
{
  $$=$2;
  int i;
  for(i=0;i<$$->size;i++){
    p_mult_cons_assign($$->polys[i],-1) ;
  }
  if(NULL!=$$->rhs)
    p_mult_cons_assign($$->rhs,-1) ;
}
|
poly '*' UNHNOW
{
