...
static Poly *getConstraintPoly(SOSProblem *const sys, const int index,
  ...
  for (i = 0; i < sys->size; ++i) {
    if (index == i) {
      if (sys->polyConstraints[i]->type == EQ) {
        ...
        Poly *poly = sosConvertPoly(X, k, blockSize[k],
                                    sosMId[sosMap[abs(blockMap[k]) - 1]]);
        return poly;
      }
    }
...
indice_t **createAllIndices(SOSProblem *const sys, int *const sosmMap,
                            ...
                            indice_t *polyVarMap[], indice_t *sosVarMap[]) {
  int i, k, p, pi, si;
  int choose;
  int psize;
  int polyVarSize;
  ...
  for (i = 0; i < sys->size; i += 1) {
    ...
    sosVarMap[i] =
        getvarElem(getSupElem((sys->polyConstraints[i])->supportId)->varId);

    multVarId[i] =
        mergeVar(sys->polys[i]->varId,
  ...
  for (p = 0; p < sys->size; p += 1) { /* polynomial index */
    ...
    while (choose) {
      ...
      if (choose) {
        for (i = 0; i < p; i += 1) {
          if (findlocation(re[i], reSize[i], temp1, varMap[i], varMap[p]) > -1)
  ...
  *b = (double *)calloc_d(((*numofCons) + 1), sizeof(double));
  ...
  if (NULL != sys->rhs) {
    ...
    for (i = 0; i < rhs->size; i++) {
      ...
      while (k < sys->size) {
        tempIndex =
            findlocation(allM[k], everyConstraintSize[k],
                         rhs->indices + i * rhsVars[0], varMap[k], rhsVars);
        if (tempIndex > -1) {
          index += tempIndex;
          break;
        } else
