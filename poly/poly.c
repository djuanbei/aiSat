...
SOSProblem * createSOSProblem(void){
  ...
  re->polyConstraints	=(PolyConstraint  **)calloc_d ( DEFAULT_SOS_PRO_SIZE, sizeof(PolyConstraint*));
  return  re;
}

SOSProblem*
createSOSProblemN ( int size )
{
  ASSERT(size>=0,"");
  ...
  re->polyConstraints	=(PolyConstraint  **)calloc_d ( size, sizeof(PolyConstraint*));
  return  re;
}
...
void setConstraint(SOSProblem * sys,const int index, Poly *poly, PolyConstraint * cons   ){
  ...
  if(NULL!=sys->polys[index])
    deletePoly(sys->polys[index]);
...
int addConstraint(SOSProblem * sys, Poly *poly, PolyConstraint * cons  ){
  while(sys->size>=sys->cap){
    ...
    sys->polyConstraints=(PolyConstraint **) realloc_d(sys->polyConstraints, sys->cap*sizeof(PolyConstraint ));
  }
...
Poly*
randPoly (const int varId, const coef_t coefDome, const int termLength,
 ...
 * ===  FUNCTION  ======================================================================
 *         Name:  copyPoly
 *  Description: poly is constant can not delete 
 * =====================================================================================
 */
Poly *
copyPoly (const  Poly * poly )
{
 ...
 */
void
polyChangeVarId(  Poly*const poly, const  int nvarId  ){
  ...
  if(newSize>oldSize){
    ...
    poly->indices=(indice_t *) realloc_d(poly->indices, poly->capacity*newSize*sizeof(indice_t) );
...
int
poly_getTotalDegree ( const Poly *poly ){
...
void
enlargePoly ( Poly *const poly  ){

  poly->capacity= (poly->capacity)*ENLARGE_RAT+1;
  poly->indices=(indice_t *) realloc_d(poly->indices,
 ...
 * ===  FUNCTION  ======================================================================
 *         Name:  updatePoly
 *  Description:  combine the terms whoes indices are same
 *
 * =====================================================================================
 */
void
updatePoly (Poly * const poly   ){
  ...
  for ( i = 1; i < poly->size; i += 1 ) {
    ...
    for ( k = 0; k < n; k += 1 ) {
      if(poly->indices[j*n+k]!=poly->indices[i*n+k])
        break;
    }
    if(n==k){
...
void
printPolyPrec ( Poly *const poly, const float minv  )
{
  int i ,j ;
  int first=1;
...
BOOL isPositive(Poly *poly){
  ...
  if(poly->coef[0]>=0&& isConstant(poly) )
    return TRUE;
  return FALSE;
}
...
/* 
 * ===  FUNCTION  ======================================================================
 *         Name:  internal_addTerm
 *  Description:  n is number of variable 
 *  poly doesn't contain key
 * =====================================================================================
 */

void
internal_addTerm ( Poly * const poly, indice_t  key[] ,const coef_t cf)
{
  int i;
 ...
 *         Name:  p_add_Term
 *      
 *  Description:  add and keep sort indices
 * =====================================================================================
 */
void
p_add_Term_assign ( Poly *const poly, indice_t const key[], const coef_t cf )
{
  int i,j;
  ...
  if(location>-1) {
    ...
    for ( i = poly->size; i > location; i -= 1 ) {
      ...
      for ( j = 0; j < varSize; j += 1 ) {
        poly->indices[i*varSize+j]=poly->indices[i*varSize-varSize+j];
      }
...
void
p_add_cons_assign ( Poly *const poly ,const coef_t cf ){
...
Poly*
p_mult_Term ( Poly const *poly ,const indice_t key[],const coef_t cf ){
...
void
p_mult_Term_assign ( Poly *const poly, indice_t const key[], const coef_t cf  )
{
  int i,j;
...
void
p_mult_cons_assign ( Poly * const poly, const coef_t cf ){
 ...
 * ===  FUNCTION  ======================================================================
 *         Name:  p_add_Poly
 *  Description:  poly1=poly1+poly2
 *  does not delete poly2
 * =====================================================================================
 */
void
p_add_Poly_assign ( Poly *const poly1, Poly  *const poly2 )
{
  ...
  for ( i = 0; i < poly2->size; i += 1 ) {
    ...
    for ( j = 0; j < p2Size; j += 1 ) {
      if(map[j]>-1)
        key[map[j]]=poly2->indices[i*p2Size+j];
    }
  ...
  if(poly1->size>=poly2->size){
    ...
    for ( i = 0; i < poly1->size; i += 1 ) {
      ...
      for ( j = 0; j < p1Size; j += 1 ) {
        if(map[j]>-1)
          key[map[j]]=poly1->indices[i*p1Size+j];
      }
 ...
 * ===  FUNCTION  ======================================================================
 *         Name:  p_mult_Poly
 *  Description:  return poly1*poly2
 * =====================================================================================
 */
Poly*
p_mult_Poly (const  Poly  *poly1,const Poly *poly2 ){
 ...
 * ===  FUNCTION  ======================================================================
 *         Name:  mergeSort
 *  Description:  list1 and list2 are order list.  re is the list merge list1 and list2.
 *  list1 and list2 doesn't delete after this function.
 *  re is always new allow memmory
 * =====================================================================================
 */
indice_t**
mergeSort ( indice_t **const list1, indice_t **const list2, const int size1, const int size2, int *const size3 ,int const n){
  ...
  *size3=0;
  ...
  /*	if ( list1==NULL ) {           list1 is a empty 
  //		re=list2;
  //		*size3=size2;
  //		return re;
  //	}
...
PolyConstraint*
createPolyConstraint(void  )
{
  PolyConstraint *re;

  re	=(PolyConstraint*) malloc_d ( sizeof(PolyConstraint) );
  return  re;
}		/* -----  end of function createCoefPoly  ----- */
...
void printPolyConstraint( PolyConstraint *const cpoly ){
  ...
  if(cpoly->type==EQ) printf ( "==0\n" );
  else if (cpoly->type==GE)printf ( ">=0 \n" ); 

  else if (cpoly->type==GT) printf ( ">\n" );

  else if (cpoly->type==NE)printf ( "!=0\n" );

}

void
deletePolyConstraint(PolyConstraint * cpoly  )
{
...
Support* createSupport( const int deg,  const int varId, const int consNum,  int *consId  ){
