...
public:
  struct Term {
    ...
    C                  cf;
    Term() {}
    Term( const vector<pair<T, T>> &k, C c = 1 )
        : key( k )
        , cf( c ) {}
  };
...
public:
  ...
  C getCF( const string &term ) const {
    ...
    for ( vector<string>::iterator it = elems.begin(); it != elems.end();
          it++ ) {
      string      str   = *it;
  ...
  string toString() const { return toString( MIN_COEF, 4 ); }
  ...
  void mult_term( const Term &term ) {
    ...
    for ( size_t i = 0; i < coef.size(); i++ ) {
      ...
      for ( size_t j = 0; j < term.key.size(); j++ ) {
        indices[ i * varNum + mapIndex[ term.key[ i ].first ] ] +=
            term.key[ i ].second;
      }
    }
  }
    ...
    if ( coef.size() >= poly2.coef.size() ) {
      ...
      for ( size_t i = 0; i < poly2.coef.size(); i += 1 ) {
        ...
        for ( j = 0; j < p2VarNum; j += 1 ) {
          if ( mapKey[ j ] > -1 )
            key[ mapKey[ j ] ] = poly2.indices[ i * p2VarNum + j ];
        }
      ...
      for ( size_t i = 0; i < coef.size(); i += 1 ) {
        ...
        for ( j = 0; j < p1VarNum; j += 1 ) {
          if ( mapKey[ j ] > -1 )
            key[ mapKey[ j ] ] = indices[ i * p1VarNum + j ];
        }
...
private:
  struct term {
    ...
    C         cf;
    term() {}
    term( const vector<T> &k, const C &c )
        : key( k )
        , cf( c ) {}
  };
