...
template <typename T> class VarTable {
private:
...
public:
  ...
  string getVarName( const int index ) const {
    ASSERT( index < (int) variateSymbol.size(), "" );
   ...
   * hold
   */
  void getConvertMap( const int oldId, const int newId,
                      vector<int> &mapIndex ) const {
    if ( oldId != 0 ) {
  ...
  template <typename T1> friend VarTable<T1> &getVarTable();
};

template <typename T> VarTable<T> &getVarTable() {
