...
/* YYPRHS[YYN] -- Index of the first RHS symbol of rule number YYN in
   YYRHS.  */
static const yytype_uint8 yyprhs[] = {0,  0,  3,  5,  11, 15, 19,
                                      23, 27, 29, 31, 33, 37};

/* YYRHS -- A `-1'-separated list of the rules' RHS.  */
static const yytype_int8 yyrhs[] = {13, 0,  -1, 14, -1, 10, 14, 11, 9,  3,
                                    -1, 10, 14, 11, -1, 14, 8,  14, -1, 14,
                                    7,  14, -1, 14, 6,  14, -1, 15, -1, 4,
                                    -1, 3,  -1, 5,  9,  3,  -1, 5,  -1};
...
yyreduce:
  ...
  switch ( yyn ) {
  case 2:
#line 68 "src/polyparser.y"
  {
    gloabl_poly = Poly_t( *( ( yyvsp[ ( 1 ) - ( 1 ) ].poly ) ) );
    delete ( yyvsp[ ( 1 ) - ( 1 ) ].poly );
    ;
  } break;

  case 3:
#line 75 "src/polyparser.y"
  {
    ( yyval.poly ) = ( yyvsp[ ( 2 ) - ( 5 ) ].poly );
    Poly_t temp( *( ( yyvsp[ ( 2 ) - ( 5 ) ].poly ) ) );
    for ( int i = 1; i < ( yyvsp[ ( 5 ) - ( 5 ) ].dilVal ); i++ ) {
      ( yyval.poly )->mult_poly( temp );
    };
  } break;

  case 4:
#line 84 "src/polyparser.y"
  {
...
#line 117 "src/polyparser.y"
  {
    ...
    for ( size_t i = 0; i < ( yyvsp[ ( 1 ) - ( 1 ) ].iVec )->size(); i += 2 ) {
      pows[ keyMap[ ( *( yyvsp[ ( 1 ) - ( 1 ) ].iVec ) )[ i ] ] ] +=
          ( *( yyvsp[ ( 1 ) - ( 1 ) ].iVec ) )[ i + 1 ];
    }
    ...
    delete ( yyvsp[ ( 1 ) - ( 1 ) ].iVec );
    ;
  } break;

  case 9:
#line 150 "src/polyparser.y"
  {
...
#line 157 "src/polyparser.y"
  ...
  case 11:
#line 167 "src/polyparser.y"
  {
    VarTable<indice_t> &table = getVarTable<indice_t>();
    int index = table.findVarIndex( *( ( yyvsp[ ( 1 ) - ( 3 ) ].varname ) ) );
    ( yyval.iVec ) = new vector<indice_t>();
    ( yyval.iVec )->push_back( index );
    ( yyval.iVec )->push_back( ( yyvsp[ ( 3 ) - ( 3 ) ].dilVal ) );
    delete ( yyvsp[ ( 1 ) - ( 3 ) ].varname );
    ;
  } break;

  case 12:
#line 177 "src/polyparser.y"
  {

    VarTable<indice_t> &table = getVarTable<indice_t>();
    int index = table.findVarIndex( *( ( yyvsp[ ( 1 ) - ( 1 ) ].varname ) ) );
    ( yyval.iVec ) = new vector<indice_t>();
    ( yyval.iVec )->push_back( index );
    ( yyval.iVec )->push_back( 1 );
    delete ( yyvsp[ ( 1 ) - ( 1 ) ].varname );
    ;
  } break;

/* Line 1267 of yacc.c.  */
#line 1498 "src/polyparser.cpp"
  default:
    break;
  }
