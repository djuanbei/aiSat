...
static void enlargevarTable(void) {
  ...
  if (table.values == NULL) {
    ...
    table.values = (indice_t *)realloc_d(
        table.values, (size_t)table.capacity * sizeof(indice_t));
    table.locMap =
        (int *)realloc_d(table.locMap, (size_t)table.mapCap * sizeof(int));
  }
}
...
void defaultVarTable(void) {
  ...
  if (FALSE == INIT) {
    ...
    //		deletevarTable();
    if (table.varCap < n) resetvarTable(n);
    return;
  }
...
BOOL addVar(const char *name) {
  ...
  if (table.varNum >= table.varCap) {
    ...
    table.varCap++;
    table.varName = (char *)realloc_d(table.varName,
                                      table.varCap * var_size * sizeof(char));
  }

  memcpy(table.varName + table.varNum * var_size, name,
  ...
  memmove(table.values + table.varNum + 2, table.values + table.varNum + 1,
          table.last - table.varNum - 1);
...
int findVarIndex(const char *name) {
  int i;
  for (i = 0; i < table.varNum; i++) {
    if (strcmp(name, table.varName + i * var_size) == 0)
...
void resetvarTable(const int n) {
  if (TRUE == INIT) {
    ...
    if (n > table.varCap) {
      table.varName =
          (char *)realloc_d(table.varName, n * var_size * sizeof(char));
      //      memset(table.varName+table.varNum*var_size ,0,
      //      (n-table.varNum)*var_size*sizeof(char));
      table.capacity += n;
      table.values = (indice_t *)realloc_d(table.values,
...
void setvarName(const int index, const char *name) {
  ...
  ASSERT(strlen(name) <= MAX_VAR_NAME_LENGTH,
         "the length of variable is too large");
  memcpy(table.varName + index * var_size, name,
         (strlen(name) + 1) * sizeof(char));
}
...
static int varContain(const int sId, const int bId) {
  int re = 0;
  int i, j;
...
indice_t *getvarElem(const int id) {
  ...
  //  if(id>table.last-2) return NULL;             /* an easy method to chech id
  //  extend bound */
...
int getvarNum(const int id) {
  ...
  //  if(id>table.last-2) return -1;             /* an easy method to chech id
  //  extend bound */
...
int getAllVarNum(void) { return table.varNum; }
