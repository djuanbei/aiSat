...
void                alloc_mat( A, pB ) struct blockmatrix A;
struct blockmatrix *pB;
{
  ...
  pB->blocks = (struct blockrec *) malloc_d( sizeof( struct blockrec ) *
                                             ( A.nblocks + 1 ) );
  /*
  ...
  for ( blk = 1; blk <= A.nblocks; blk++ ) {
    ...
    case DIAG:
      pB->blocks[ blk ].data.vec = (double *) malloc_d(
          sizeof( double ) * ( A.blocks[ blk ].blocksize + 1 ) );

      break;
    case MATRIX:
      pB->blocks[ blk ].data.mat = (double *) malloc_d(
          sizeof( double ) * ( A.blocks[ blk ].blocksize ) *
          ( A.blocks[ blk ].blocksize ) );
      break;
    default:
...
void free_mat( A ) struct blockmatrix A;
{
  int blk;
