...

{
  ...
  for (i = 1; i <= nblocks; i++) isdiag[i] = 1;
  ...
  pC->nblocks = nblocks;
  pC->blocks =
      (struct blockrec *)malloc_d((nblocks + 1) * sizeof(struct blockrec));
  ...
  *pn = 0;
    ...
    for (blk = 1; blk <= nblocks; blk++) {
      ...
      *pn = *pn + abs(blksz);
      ...
      if (blksz < 0) {
        ...
        for (i = 1; i <= abs(blksz); i++) pC->blocks[blk].data.vec[i] = 0.0;
  ...
  do {
    ...
    if ((matno < 0) || (matno > *pk) || (blkno < 1) || (blkno > nblocks) ||
  ...
  for (i = 1; i <= *pk; i++) {
    ...
    while (p != NULL) {
       ...
       */
      p->entries = (double *)malloc_d((p->numentries + 1) * sizeof(double));
