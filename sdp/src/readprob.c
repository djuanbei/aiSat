...
int  read_prob( fname, pn, pk, pC, pa, pconstraints, printlevel ) char *fname;
int *pn;
int *pk;
...

{
  ...
  buf = (char *) malloc_d( buflen * sizeof( char ) );
  ...
  isdiag = (int *) malloc_d( ( nblocks + 1 ) * sizeof( int ) );
  for ( i = 1; i <= nblocks; i++ )
    isdiag[ i ] = 1;
  ...
  pC->nblocks = nblocks;
  pC->blocks  = (struct blockrec *) malloc_d( ( nblocks + 1 ) *
                                             sizeof( struct blockrec ) );

  /*
   * Allocate space for the constraints.
   */

  myconstraints = (struct constraintmatrix *) malloc_d(
      ( *pk + 1 ) * sizeof( struct constraintmatrix ) );
  ...
  *pa = (double *) malloc_d( ( *pk + 1 ) * sizeof( double ) );
  ...
  *pn = 0;
    ...
    for ( blk = 1; blk <= nblocks; blk++ ) {
      ...
      *pn = *pn + abs( blksz );
      ...
      if ( blksz < 0 ) {
        ...
        pC->blocks[ blk ].data.vec =
            (double *) malloc_d( ( 1 + abs( blksz ) ) * sizeof( double ) );
        ...
        pC->blocks[ blk ].data.mat =
            (double *) malloc_d( ( blksz * blksz ) * sizeof( double ) );
  ...
  ret =
      fscanf( fid, "%d %d %d %d %le ", &matno, &blkno, &indexi, &indexj, &ent );
  ...
  do {
    ...
    if ( ( matno < 0 ) || ( matno > *pk ) || ( blkno < 1 ) ||
         ( blkno > nblocks ) || ( indexi < 1 ) ||
         ( indexi > pC->blocks[ blkno ].blocksize ) || ( indexj < 1 ) ||
         ( indexj > pC->blocks[ blkno ].blocksize ) ) {
    ...
    if ( matno != 0 ) {
      if ( ent != 0.0 )
        countentry( myconstraints, matno, blkno,
                    pC->blocks[ blkno ].blocksize );
    } else {
      /*
       * An entry in C. ignore it for now.
       */
    };
    ret = fscanf( fid, "%d %d %d %d %le", &matno, &blkno, &indexi, &indexj,
                  &ent );
  } while ( ret == 5 );
  ...
  for ( i = 1; i <= *pk; i++ ) {
    ...
    while ( p != NULL ) {
       ...
       */
      p->entries =
          (double *) malloc_d( ( p->numentries + 1 ) * sizeof( double ) );

      p->iindices = (int *) malloc_d( ( p->numentries + 1 ) * sizeof( int ) );

      p->jindices = (int *) malloc_d( ( p->numentries + 1 ) * sizeof( int ) );
  ...
  do {
     ...
     */
    if ( ( indexi != indexj ) && ( ent != 0.0 ) ) isdiag[ blkno ] = 0;
  ...
  for ( i = 1; i <= nblocks; i++ ) {
    if ( ( pC->blocks[ i ].blockcategory != DIAG ) && ( isdiag[ i ] == 1 ) &&
         ( pC->blocks[ i ].blocksize > 1 ) ) {
      /*
      ...
      tempdiag = (double *) malloc_d( ( blksz + 1 ) * sizeof( double ) );
      for ( j = 1; j <= blksz; j++ )
  ...
  for ( i = 1; i <= *pk; i++ ) {
    ...
    while ( p != NULL ) {
      ...
      if ( pC->blocks[ p->blocknum ].blockcategory == DIAG ) p->issparse = 1;
...
void skip_to_end_of_line( fid ) FILE *fid;
{
  char c;
...
int   get_line( fid, buffer, bufsiz ) FILE *fid;
char *buffer;
int   bufsiz;
{
  int  i;
  ...
  while ( c != '\n' ) {
    ...
    if ( k >= bufsiz ) {
      printf(
          "Line too long in input file!  Adjust BUFFERSIZ in readprob.c\n" );
      return ( 1 );
    };
  };
  buffer[ k ]     = '\n';
...
int max_line_length( fid ) FILE *fid;
{
  int maxlen;
  int k;
  int c;
...
void countentry( constraints, matno, blkno,
                 blocksize ) struct constraintmatrix *constraints;
int  matno;
int  blkno;
int  blocksize;
{
  ...
  if ( p == NULL ) {
     ...
     */
    p = (struct sparseblock *) malloc_d( sizeof( struct sparseblock ) );
     ...
     */
    q = (struct sparseblock *) malloc_d( sizeof( struct sparseblock ) );
...
void   addentry( constraints, matno, blkno, indexi, indexj,
               ent ) struct constraintmatrix *constraints;
int    matno;
int    blkno;
int    indexi;
int    indexj;
