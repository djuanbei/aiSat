    ...
    while ( ptr != NULL ) {
      ...
      case DIAG:
        for ( i = 1; i <= A.blocks[ blk ].blocksize; i++ )
          C.blocks[ blk ].data.vec[ i ] = scale1 *
...
#ifdef USEOPENMP
#pragma omp parallel default( none ) private( i, ii, p, q, thread_num,         \
                                              total_threads, temp )            \
    shared( ptr, A, B, C, blk, blksize, scale1 )
          {
            ...
            for ( ii = 1; ii <= ptr->numentries; ii++ ) {
              ...
              if ( ( q % total_threads ) == thread_num ) {
                p = ptr->iindices[ ii ];
                temp =
                    scale1 * B.blocks[ blk ].data.mat[ ijtok( p, q, blksize ) ];
                for ( i = 1; i <= blksize; i++ )
                  C.blocks[ blk ].data.mat[ ijtok( i, q, blksize ) ] +=
                      temp * A.blocks[ blk ].data.mat[ ijtok( i, p, blksize ) ];
              };
            };
          }
#pragma omp barrier
    ...
    for ( blk = 1; blk <= C.nblocks; blk++ ) {
      ...
      case DIAG:
        for ( i = 1; i <= C.blocks[ blk ].blocksize; i++ )
          C.blocks[ blk ].data.vec[ i ] =
              scale2 * C.blocks[ blk ].data.vec[ i ];
        break;
      case MATRIX:
#pragma omp parallel for schedule( dynamic, 64 ) default( none ) private(      \
    i, j ) shared( blk, C, scale2 )
        for ( j = 1; j <= C.blocks[ blk ].blocksize; j++ )
          for ( i = 1; i <= C.blocks[ blk ].blocksize; i++ )
            C.blocks[ blk ]
                .data.mat[ ijtok( i, j, C.blocks[ blk ].blocksize ) ] =
                scale2 *
                C.blocks[ blk ]
                    .data.mat[ ijtok( i, j, C.blocks[ blk ].blocksize ) ];
        break;
...
void mat_multspa( scale1, scale2, A, B, C, fill ) double scale1, scale2;
    ...
    shared( ptr, A, B, C, blk, blksize, scale1 )
          {
            ...
            for ( ii = 1; ii <= ptr->numentries; ii++ ) {
              ...
              if ( ( p % total_threads ) == thread_num ) {
                q = ptr->jindices[ ii ];
                temp =
                    scale1 * A.blocks[ blk ].data.mat[ ijtok( p, q, blksize ) ];
                for ( i = 1; i <= ptr->blocksize; i++ )
                  C.blocks[ blk ].data.mat[ ijtok( p, i, blksize ) ] +=
                      temp * B.blocks[ blk ].data.mat[ ijtok( i, q, blksize ) ];
              };
            };
          }
#pragma omp barrier
...
#pragma omp parallel for default( none )                                       \
    schedule( dynamic, 64 ) private( i, j ) shared( blk, C, scale2 )
        for ( j = 1; j <= C.blocks[ blk ].blocksize; j++ )
          for ( i = 1; i <= C.blocks[ blk ].blocksize; i++ )
            C.blocks[ blk ]
                .data.mat[ ijtok( i, j, C.blocks[ blk ].blocksize ) ] =
...
void mat_multspc( scale1, scale2, A, B, C, fill ) double scale1, scale2;
struct blockmatrix      A, B, C;
struct constraintmatrix fill;
{
  int                 blk, i, j, ii;
...
#endif
#pragma omp parallel for schedule( dynamic, 64 ) default( none ) private(      \
    i, ii, p, q, temp ) shared( ptr, A, B, C, blk, blksize, scale1 )
          for ( ii = 1; ii <= ptr->numentries; ii++ ) {
            p    = ptr->iindices[ ii ];
            q    = ptr->jindices[ ii ];
            temp = 0;
