...
int                      printlevel;
{
      ...
      for ( i = 1; i <= C.blocks[ blk ].blocksize; i++ )
        for ( j = 1; j <= C.blocks[ blk ].blocksize; j++ ) {
          if ( ( C.blocks[ blk ]
               ...
               ( i == j ) )
            work1.blocks[ blk ]
                .data.mat[ ijtok( i, j, C.blocks[ blk ].blocksize ) ] = 1.0;
        };
      break;
    case PACKEDMATRIX:
    default:
    ...
    while ( ptr != NULL ) {
      ...
      case MATRIX:
        for ( j = 1; j <= ptr->numentries; j++ ) {
          ...
          q = ptr->jindices[ j ];
          work1.blocks[ blk ]
              .data.mat[ ijtok( p, q, C.blocks[ blk ].blocksize ) ] = 1.0;
          work1.blocks[ blk ]
              .data.mat[ ijtok( q, p, C.blocks[ blk ].blocksize ) ] = 1.0;
        };
        break;
      case PACKEDMATRIX:
      default:
  ...
  while ( ptr != NULL ) {
    ...
    case DIAG:
      ...
      ptr->iindices =
          (int *) malloc_d( ( ptr->blocksize + 1 ) * sizeof( int ) );
    ...
    case MATRIX:
      ...
      ptr->iindices =
          (int *) malloc_d( ( ptr->numentries + 1 ) * sizeof( int ) );
  ...
  while ( ptr != NULL ) {
    ...
    if ( printlevel >= 3 )
      printf(
          "Block %d, Size %d, Fill %d, %.2f \n", blk, C.blocks[ blk ].blocksize,
          ptr->numentries,
