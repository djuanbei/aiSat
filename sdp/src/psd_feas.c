 ...
 */
double pinfeas(k, constraints, X, a, workvec) int k;
...
double *workvec;
{
  ...
  nrma = norm2(k, a + 1);
  for (i = 1; i <= k; i++) workvec[i] = workvec[i] - a[i];
  nrme = norm2(k, workvec + 1);

  return (nrme / (1.0 + nrma));
}

double dinfeas(k, C, constraints, y, Z, work1) int k;
...
double dimacserr3(k, C, constraints, y, Z, work1) int k;
struct blockmatrix C;
struct constraintmatrix *constraints;
double *y;
struct blockmatrix Z;
