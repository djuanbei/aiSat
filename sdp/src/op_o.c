...
void op_o( k, constraints, byblocks, Zi, X, O, work1, work2 ) int k;
...
struct blockmatrix       work2;
{
  ...
  if ( max_blknum > 0 ) {
    work = (double **) malloc_d( sizeof( double * ) * ( max_threads * 2 + 1 ) );
    work[ 1 ] = work1.blocks[ max_blknum ].data.mat;
    work[ 2 ] = work2.blocks[ max_blknum ].data.mat;
    for ( i = 1; i < max_threads; i++ ) {
      work[ i * 2 + 1 ] = (double *) malloc_d(
          sizeof( double ) * X.blocks[ max_blknum ].blocksize *
          X.blocks[ max_blknum ].blocksize );
      work[ i * 2 + 2 ] = (double *) malloc_d(
          sizeof( double ) * X.blocks[ max_blknum ].blocksize *
          X.blocks[ max_blknum ].blocksize );
    };
  };
...
#pragma omp parallel for schedule( dynamic, 64 ) default( none )               \
    shared( k, constraints, byblocks, Zi, X, O, work1, work2, work,            \
            ldam ) private( j, ii, jj, blocksize, p, q, r, s, ptri, ptrj,      \
                            blocknum, contrib, Ziblk, Xblk, workblk, work2blk, \
                            enti, entj, scale1, scale2, thread_num )
  for ( i = 1; i <= k; i++ ) {
#ifdef USEOPENMP
#ifdef SETNUMTHREADS
    ...
    while ( ptri != NULL ) {
      ...
      /* Diagonal blocks */
      if ( ptri->issparse == 1 && X.blocks[ blocknum ].blockcategory == DIAG ) {
        ...
        while ( ptrj != NULL ) {
          ...
          p       = 1;
          q       = 1;
          while ( ( p <= ptri->numentries ) && ( q <= ptrj->numentries ) ) {
 ...
 */
            ...
            for ( ii = 1; ii <= ptri->numentries; ii++ ) {
              ...
              if ( p == q ) {
                for ( jj = 1; jj <= ptrj->numentries; jj++ ) {
                  ...
                  if ( r == s ) {
                    /* here p==q, r==s */
                    contrib += enti * entj * Ziblk[ ijtok( r, q, blocksize ) ] *
      ...
      /* Dense matrix block */
        ...
        for ( ii = 1; ii <= ptri->numentries; ii++ ) {
          ...
          if ( p != q ) workblk[ ijtok( q, p, blocksize ) ] = enti;
        }
...
#ifdef USEATLAS
        mat_mult_rawatlas( blocksize, scale1, scale2, Ziblk, workblk,
                           work2blk );
        ...
        while ( ptrj != NULL ) {
           ...
           * equal to j
           */
          if ( ptrj->issparse == 1 || ptri->numentries > ptrj->numentries ||
 ...
 */
            ...
            for ( ii = 1; ii <= ptrj->numentries; ii++ ) {
              ...
              p    = ijtok( ptrj->iindices[ ii ], ptrj->jindices[ ii ],
                         blocksize );
              q    = ijtok( ptrj->jindices[ ii ], ptrj->iindices[ ii ],
                         blocksize );
