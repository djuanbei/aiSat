 ...
 */
  ...
  if (max_blknum > 0) {
    ...
    for (i = 1; i < max_threads; i++) {
      work[i * 2 + 1] =
          (double *)malloc_d(sizeof(double) * X.blocks[max_blknum].blocksize *
...
#pragma omp parallel for schedule(dynamic, 64) default(none) \
  ...
  for (j = 1; j <= k; j++)
    for (i = 1; i <= k; i++) O[ijtok(i, j, ldam)] = 0.0;
...
#pragma omp parallel for schedule(dynamic, 64) default(none) shared(           \
    k, constraints, byblocks, Zi, X, O, work1, work2, work,                    \
    ldam) private(j, ii, jj, blocksize, p, q, r, s, ptri, ptrj, blocknum,      \
                  contrib, Ziblk, Xblk, workblk, work2blk, enti, entj, scale1, \
                  scale2, thread_num)
  for (i = 1; i <= k; i++) {
#ifdef USEOPENMP
#ifdef SETNUMTHREADS
            ...
            if (ptri->iindices[p] < ptrj->iindices[q]) {
              ...
              if (ptri->iindices[p] > ptrj->iindices[q]) {
                ...
                contrib += ptri->entries[p] * ptrj->entries[q] *
                           Ziblk[ptri->iindices[p]] * Xblk[ptri->iindices[p]];
                p = p + 1;
                q = q + 1;
              };
            };
...
#pragma omp parallel for schedule(dynamic, 64) default(none) \
                                      shared(O, ldam, k) private(j, i)
  for (j = 2; j <= k; j++)
    for (i = 1; i < j; i++) O[ijtok(i, j, ldam)] += O[ijtok(j, i, ldam)];
