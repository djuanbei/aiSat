...
struct blockmatrix *pZ;
{
  ...
  alloc_mat( C, pX );
  alloc_mat( C, pZ );
  *py = (double *) malloc_d( sizeof( double ) * ( k + 1 ) );
  ...
  do {
    ...
    if ( matno == 1 ) {
      ...
      case DIAG:
        ...
        break;
      case MATRIX:
        pZ->blocks[ blkno ].data.mat[ ijtok(
            indexi, indexj, pZ->blocks[ blkno ].blocksize ) ] = ent;
        pZ->blocks[ blkno ].data.mat[ ijtok(
            indexj, indexi, pZ->blocks[ blkno ].blocksize ) ] = ent;
        break;
      default:
      ...
      case DIAG:
        ...
        break;
      case MATRIX:
        pX->blocks[ blkno ].data.mat[ ijtok(
            indexi, indexj, pX->blocks[ blkno ].blocksize ) ] = ent;
        pX->blocks[ blkno ].data.mat[ ijtok(
            indexj, indexi, pX->blocks[ blkno ].blocksize ) ] = ent;
        break;
      default:
        printf( "Illegal block type! \n" );
        exit( 12 );
