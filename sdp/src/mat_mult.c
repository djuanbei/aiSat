...
void               mat_mult( scale1, scale2, A, B, C ) double scale1, scale2;
      ...
      if ( scale2 != 0.0 ) {
        for ( i = 1; i <= A.blocks[ blk ].blocksize; i++ ) {
          C.blocks[ blk ].data.vec[ i ] =
              scale1 * A.blocks[ blk ].data.vec[ i ] *
...
void    mat_mult_raw( n, scale1, scale2, ap, bp, cp ) int n;
double  scale1;
double  scale2;
double *ap;
double *bp;
double *cp;
{ MY_DGEMM( "N", "N", &n, &n, &n, &scale1, ap, &n, bp, &n, &scale2, cp, &n ); }

#ifdef USEATLAS

void    mat_mult_rawatlas( n, scale1, scale2, ap, bp, cp ) int n;
double  scale1;
double  scale2;
double *ap;
double *bp;
double *cp;
{
  ...
  void ATL_dgemm( enum CBLAS_TRANSPOSE transa, enum CBLAS_TRANSPOSE transb,
                  int m, int n, int k, double scale1, double *ap, int l,
                  double *bp, int p, double scale2, double *cp, int q );

  ATL_dgemm( CblasNoTrans, CblasNoTrans, n, n, n, scale1, ap, n, bp, n, scale2,
             cp, n );
}
