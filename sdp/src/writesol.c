...
int                write_sol( fname, n, k, X, y, Z ) const char *fname;
int                n;
int                k;
  ...
  for ( blk = 1; blk <= Z.nblocks; blk++ ) {
    ...
    case DIAG:
      for ( i = 1; i <= Z.blocks[ blk ].blocksize; i++ ) {
        ...
        if ( ent != 0.0 ) fprintf( fid, "1 %d %d %d %.18e \n", blk, i, i, ent );
      };
      break;
    case MATRIX:
      for ( i = 1; i <= Z.blocks[ blk ].blocksize; i++ )
        for ( j = i; j <= Z.blocks[ blk ].blocksize; j++ ) {
          ent = Z.blocks[ blk ]
                    .data.mat[ ijtok( i, j, Z.blocks[ blk ].blocksize ) ];
          if ( ent != 0.0 )
  ...
  for ( blk = 1; blk <= X.nblocks; blk++ ) {
    ...
    case DIAG:
      for ( i = 1; i <= X.blocks[ blk ].blocksize; i++ ) {
        ...
        if ( ent != 0.0 ) fprintf( fid, "2 %d %d %d %.18e \n", blk, i, i, ent );
      };
      break;
    case MATRIX:
      for ( i = 1; i <= X.blocks[ blk ].blocksize; i++ )
        for ( j = i; j <= X.blocks[ blk ].blocksize; j++ ) {
          ent = X.blocks[ blk ]
                    .data.mat[ ijtok( i, j, X.blocks[ blk ].blocksize ) ];
          if ( ent != 0.0 )
