...
double Fnorm( A ) struct blockmatrix A;
{
  int    blk;
  double nrm;
  double temp;
...
double Knorm( A ) struct blockmatrix A;
{
  int    blk;
  double nrm;
  double temp;
...
double mat1norm( A ) struct blockmatrix A;
{
  int    blk;
  double nrm;
  double temp;
...
double matinfnorm( A ) struct blockmatrix A;
{
  int    blk;
  int    i;
  double nrm;
  ...
  for ( blk = 1; blk <= A.nblocks; blk++ ) {
    ...
    case DIAG:
      ...
      break;
    case MATRIX:
      for ( i = 0; i < A.blocks[ blk ].blocksize * A.blocks[ blk ].blocksize;
            i++ ) {
        if ( fabs( A.blocks[ blk ].data.vec[ i ] ) > nrm )
          nrm = fabs( A.blocks[ blk ].data.vec[ i ] );
      };
