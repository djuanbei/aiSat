...
int PolyConstraintTable::addcoefElem(const PolyConstraintType type, const int mId) {
...
int PolyConstraintTable::findcoefElem(const PolyConstraintType type, const int mId) {
  int i;
  for (i = 0; i < values.size(); i += 1) {
    if (values[i]->type == type && values[i]->supportId == mId)
      return i;
  }
