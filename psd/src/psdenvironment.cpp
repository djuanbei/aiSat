...
int PSDEnvironment::addSupport( const vector<int> &vars,
                                const vector<int> &degs ) {
  ...
  if ( 0 != gLength * varNum ) {
    GSUP =
        (indice_t *) realloc_d( Gsup, gLength * varNum * sizeof( indice_t ) );
  } else {
    GSUP = Gsup;
  }
...
bool PSDEnvironment::addVarVec( const string &     strID,
...
bool PSDEnvironment::addMonoElem( const string &strID,
                                  const int     monomialListID ) {
...
bool PSDEnvironment::addPolyConsElem( const string &        strID,
                                      const PolyConstraint &polyCons ) {
