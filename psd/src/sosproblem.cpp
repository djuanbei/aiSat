  ...
  for ( i = 0; i < size; ++i ) {
    if ( index == i ) {
      if ( polyConstraints[ i ].type == EQ ) {
        Poly_t posPoly =
            sosConvertPoly( X, k, blockSize[ k ],
        ...
        return sosConvertPoly( X, k, blockSize[ k ],
                               sosMId[ sosMap[ abs( blockMap[ k ] ) - 1 ] ] );
      }
    }
 ...
 * @param X
 */
void SOSProblem::wellform( const int sep, const int sosMId[],
                           const int sosMap[], const int blockSize[],
  ...
  for ( i = 0; i < polys.size(); i += 1 ) {
    ...
    if ( polyConstraints[ i ].type == EQ ) {
      Poly_t posPoly = sosConvertPoly(
          X, k, blockSize[ k ], sosMId[ sosMap[ abs( blockMap[ k ] ) - 1 ] ] );

      k++;
      Poly_t negPoly = sosConvertPoly(
          X, k, blockSize[ k ], sosMId[ sosMap[ abs( blockMap[ k ] ) - 1 ] ] );
      negPoly.mult( -1.0 );
...
void createBlockMatrixC( int blockSize[], int const blockNum, Blockmatrix *C ) {
  ...
  for ( i = 0; i < blockNum; i += 1 ) {
    ...
    C->blocks[ i + 1 ].data.mat      = (double *) calloc(
        ( size_t )( blockSize[ i ] * blockSize[ i ] ), sizeof( double ) );
 ...
 * @return
 */
indice_t **SOSProblem::createAllIndices(
    int *const sosmMap, indice_t **const SOSM, int const *lengthM, int reSize[],
    vector<vector<indice_t>> &varMap, vector<vector<indice_t>> &polyVarMap,
    vector<vector<indice_t>> &sosVarMap ) const {
  int i, k, p, pi, si;
  int choose;
  int psize;
  int polyVarSize;
  ...
  for ( i = 0; i < size; i += 1 ) {
    ...
    getVarTable<indice_t>().getVarVec( polys[ i ].getVarId(), polyVarMap[ i ] );
    ...
    getVarTable<indice_t>().getVarVec( multVarId[ i ], varMap[ i ] );
  ...
  for ( i = 0; i < size; i += 1 ) {
    re[ i ] = (indice_t *) malloc_d( varSize[ i ] * capacity[ i ] *
                                     sizeof( indice_t ) );
  }
    ...
    while ( choose ) {
      ...
      for ( k = 0; k < psize; k += 1 ) { /* polynomial term size */
        ...
        if ( index[ k ] < lengthM[ sosmMap[ p ] ] ) {
          polyVarSize =
              getVarTable<indice_t>().getVarNum( polys[ p ].getVarId() );
          sosVarSize = getVarTable<indice_t>().getVarNum(
              SUPPORT_TABLE.getSupElem( ( polyConstraints[ p ] ).supportId )
                  ->varId );
      ...
      if ( choose ) {
        for ( i = 0; i < p; i += 1 ) {
          if ( findlocation( re[ i ], reSize[ i ], temp1,
                             (int) varMap[ i ].size(), &( varMap[ i ] )[ 0 ],
                             (int) varMap[ p ].size(),
                             &( varMap[ p ] )[ 0 ] ) > -1 )
...
void addMonomial( indice_t **const array, const indice_t *element,
  ...
  if ( *size == *capacity ) {
    ...
    *array    = (indice_t *) realloc_d( *array, ( size_t )( *capacity ) * n *
                                                 sizeof( indice_t ) );
  }
 ...
 * @return
 */
Constraintmatrix *SOSProblem::createConstraintmatrx(
    int *const numofCons, int sosMId[], int sosmMap[], int blockSize[],
    int blockMap[], int *blockNum, double **b ) const {
  ...
  /*    int sosMId[sys->size];
   */
  int                      polySize = (int) polys.size();
  int                      diffSOSMIdSize;
  int                      everyConstraintSize[ polySize ];
  vector<vector<indice_t>> varMap( polySize );
  vector<vector<indice_t>> polyVarMap( polySize );
  vector<vector<indice_t>> sosVarMap( polySize );
  ...
  *b = (double *) calloc_d( ( ( *numofCons ) + 1 ), sizeof( double ) );
  ...
  if ( rhs.getInit() ) {
    ...
    getVarTable<indice_t>().getVarVec( rhs.getVarId(), rhsVars );
    ...
    for ( i = 0; i < len; i++ ) {
      ...
      while ( k < polySize ) {
        tempIndex =
            findlocation( allM[ k ], everyConstraintSize[ k ], &key[ 0 ],
                          (int) varMap[ k ].size(), &( varMap[ k ] )[ 0 ],
                          (int) rhsVars.size(), &rhsVars[ 0 ] );
        if ( tempIndex > -1 ) {
          index += tempIndex;
          break;
        } else {
  ...
  block = (SparseSOSblock ***) malloc_d( ( *numofCons ) *
                                         sizeof( SparseSOSblock ** ) );

  for ( i = 0; i < *numofCons; i += 1 ) {
    block[ i ] = (SparseSOSblock **) malloc_d( ( *blockNum ) *
                                               sizeof( SparseSOSblock * ) );
    ...
    for ( j = 0; j < *blockNum; j += 1 ) {
       ...
       *-----------------------------------------------------------------------------*/
      block[ i ][ j ] = new SparseSOSblock(); /* every polynomial with regard to
                                                 a constraint block */
    }
  }
  ...
  for ( i = 0; i < *blockNum; i += 1 ) {
    ...
    for ( h = 0; h < size; h += 1 ) {
      for ( j = 0; j < lengthM[ sosmMap[ p ] ];
        ...
        while ( k < size ) {
          tempIndex =
              findlocation( allM[ k ], everyConstraintSize[ k ], key,
                            (int) varMap[ k ].size(), &( varMap[ k ] )[ 0 ],
                            (int) varMap[ p ].size(), &( varMap[ p ] )[ 0 ] );
          if ( tempIndex > -1 ) {
            index += tempIndex;
            break;
          } else
  ...
  for ( i = 0; i < *numofCons; i += 1 ) {
    for ( j = *blockNum - 1; j >= 0;
      ...
      blockptr = createSparseblock( j + 1, blockSize[ j ], i + 1,
                                    sum ); /* j+1  in the first location  system
                                              and index start form 1 */

      tempI = 1;

      for ( k = 0; k < block[ i ][ j ]->index.size(); k += 1 ) {
        for ( h = 0; h < AM[ p ][ block[ i ][ j ]->index[ k ] ]->size();
              h += 1 ) {
          blockptr->iindices[ tempI ] =
              ( *( AM[ p ][ block[ i ][ j ]->index[ k ] ] ) )[ h ].row +
              1; /* row start form 1 */
          blockptr->jindices[ tempI ] =
              ( *( AM[ p ][ block[ i ][ j ]->index[ k ] ] ) )[ h ].col +
  ...
  deleteAllIndices( allM, polySize );
...
int SOSProblem::getSOSMsize( int sosmMap[], int sosMId[] ) const {
  int size = polys.size();
  int re   = 0;
  int i    = 0;
  int j    = 0;
